...
async function testFinalIntercept() {
  console.log('=== 最终演示：访问职位详情页面并拦截 API ===\n');

  try {
    // 连接到现有的 Chrome 浏览器（端口 9222）
    const client = new CDPClient({
      port: 9222,
      name: 'final-intercept-example',
    ...
    // 查找 jobDetail.json 请求
    const jobDetailRequest = har?.log.entries.find((entry: any) =>
      entry.request.url.includes('jobDetail.json')
    );

    if (jobDetailRequest) {
      console.log('✓ 找到 jobDetail.json 请求！');
      console.log(`  URL: ${jobDetailRequest.request.url}`);
      console.log(`  响应状态: ${jobDetailRequest.response.status}`);

      if (jobDetailRequest.response.text) {
        const data = JSON.parse(jobDetailRequest.response.text);
        ...
        if (data.zpData) {
          ...
          if (data.zpData.companyInfo) {
            console.log(`\n  公司信息:`);
            console.log(`    公司名称: ${data.zpData.companyInfo.companyName || 'N/A'}`);
            console.log(`    公司规模: ${data.zpData.companyInfo.companySize || 'N/A'}`);
            console.log(`    行业: ${data.zpData.companyInfo.industry || 'N/A'}`);
          }
        }

        // 保存数据
        fs.writeFileSync('click-intercept-result.json', jobDetailRequest.response.text);
        console.log('\n  ✓ 数据已保存到 click-intercept-result.json');
      }
    } else {
      console.log('✗ 没有找到 jobDetail.json 请求');
    }
...
testFinalIntercept().catch(console.error);