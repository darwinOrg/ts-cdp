...
async function testWatchXHRWithUrls() {
  console.log('=== 监听 XHR 请求（指定 URL） ===\n');

  try {
    // 连接到现有的 Chrome 浏览器（端口 9222）
    const client = new CDPClient({
      port: 9222,
      name: 'watch-xhr-with-urls-example',
      watchUrls: ['zhipin.com'] // 监听所有包含 zhipin.com 的请求
    });
    ...
    if (har2 && har2.log.entries.length > 0) {
      const newEntries = har2.log.entries.slice(har?.log.entries.length || 0);
...
testWatchXHRWithUrls().catch(console.error);